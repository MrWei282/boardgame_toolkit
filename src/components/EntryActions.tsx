...
export function EntryActions({
  ...
  return (
    <div className="flex shrink-0 items-start gap-0.5">
      <button onClick={onEdit} aria-label={t('entry.edit')} className={`${btn} text-muted`}>
        ✎
      </button>
      <button
        onClick={onTogglePin}
        aria-label={pinned ? t('entry.unpin') : t('entry.pin')}
      ...
      <button
        ...
        aria-label={hidden ? t('entry.unstrike') : t('entry.strike')}
        className={`${btn} ${hidden ? 'text-ink' : 'text-muted'}`}
      >
        <span className="line-through">S</span>
      </button>
      <button onClick={() => setConfirming(true)} aria-label={t('entry.delete')} className={`${btn} text-muted`}>
        ✕
      </button>
      ...
      {confirming &&
        createPortal(
          ...
          >
            ...
            >
              <p className="text-sm">{deleteMessage ?? t('entry.deleteEntry')}</p>
